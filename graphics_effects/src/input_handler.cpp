...
input_handler::input_handler(void)
	: key_map { 0 }, mouse_button_map { 0 }, window_resized (false)
{
}
auto input_handler::key(int32_t glfw_key) -> bool &
{
	return key_map[glfw_key];
}
auto input_handler::resized(void) -> bool &
{
...
auto input_handler::mouse_button(int32_t glfw_mb) -> bool &
{
...
auto input_handler::window_size(void) -> glm::vec2 
{
	...
	return construct<glm::vec2, float>(width, height);
}
