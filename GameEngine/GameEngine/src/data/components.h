...
/* new component system architecture */
template <typename T, typename Data> struct component;
...
private:
	friend class scene;
	friend class component_system<Data>;
	
	component_system<Data> * component_systems;
...
public:
	...
	template <typename T> auto get_component(void) -> component<T, Data> &
	{
	...
	template <typename T> auto add_component(component<T, Data> & comp) -> void
	{
		...
		component_systems->add_component<T>(*this, comp);
	}
...
template <typename Data> struct icomponent
{
protected:
	friend class component_system<Data>;
	friend class object<Data>;
...
template <typename T, typename Data> struct component : icomponent<Data>
{
public:
...
public:
	template <typename T> auto add(component<T, Data> const & new_item) -> i32
	{
		return components.add(new_item);
	}
...
public:
	...
	template <typename T> auto add_component(object<Data> & subject, component<T, Data> & comp) -> void
	{
		...
		comp.entity_index = subject.id;
		i32 comp_at = sys.add(comp);
