...
auto light_handler::create(texture * shadow_map, camera & cam) -> void
{
...
auto light_handler::update_buffer(camera & cam) -> void
{
	...
	light_info_shader_data data { 
		cam.get_view_matrix() * glm::vec4(light.light_position, 1.0f)
