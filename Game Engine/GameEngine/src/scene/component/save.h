...
struct save_apply_func : apply_func
{
	nlohmann::json * handle;

	save_apply_func(nlohmann::json * handle)
		: handle(handle)
	{
	}
	...
	auto apply(class scene & dest, nlohmann::json::iterator & it, game_object & obj) -> void override
	{
		if (it.value())
		{
			component<struct component_save, game_object_data> save_component(handle);

			obj.add_component(save_component);
		}
	}
};