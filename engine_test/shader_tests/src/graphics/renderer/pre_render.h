...
struct material : renderer_pre_render
{
	...
	material(glm::vec3 const & ambient_ref, glm::vec3 const & diff_ref, glm::vec3 const & spec_ref, f32 shiny)
		: ambient_reflectivity(ambient_ref), diffuse_reflectivity(diff_ref), specular_reflectivity(spec_ref), shininess_factor(shiny)
	{
	}
