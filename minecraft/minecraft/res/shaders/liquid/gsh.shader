...
FaceTextureMap GenerateTextureMap(int index)
{
	...
	FaceTextureMap ftm;
	float tdu = mod(texture_data, 16) / 16.0f;
	float tdv = floor(texture_data / 16.0f) / 16.0f;
...
void CreateVertex(vec3 offset, vec3 n, vec2 tcoords, int f)
{
	...
	vec3 world_position = pass_world_position[0] + actual_offset - vec3(0.0f, 0.07f, 0.0f);
